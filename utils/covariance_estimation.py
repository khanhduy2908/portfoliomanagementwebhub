...
def compute_cov_matrices(selected_combinations, returns_df):
    cov_matrix_dict = {}
    for combo in selected_combinations:
        tickers_subset = list(combo)  # combo is a tuple like ('VNM', 'FPT', 'MWG')
        try:
            cov_matrix = compute_shrunk_cov_matrix(tickers_subset, returns_df)
            combo_key = '-'.join(combo)
            cov_matrix_dict[combo_key] = cov_matrix
        except Exception as e:
