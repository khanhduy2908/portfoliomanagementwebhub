...
def run(hrp_result_dict, adj_returns_combinations, cov_matrix_dict,
    ...
    # 1. Chọn portfolio tốt nhất theo Sharpe Ratio
    ...
    # 2. Tính toán lợi suất kỳ vọng và phương sai
    ...
    # 4. Giới hạn tỷ trọng tài sản không rủi ro theo risk_score
    if 10 <= risk_score <= 17:
        max_rf_ratio = 0.40  # Low risk tolerance
    elif 18 <= risk_score <= 27:
        max_rf_ratio = 0.20  # Medium
    elif 28 <= risk_score <= 40:
        max_rf_ratio = 0.10  # High risk tolerance
    else:
    ...
    # 5. Tối ưu utility theo hàm tối ưu hóa
    def neg_utility(y):
    ...
    # 6. Tính toán các thông số danh mục
