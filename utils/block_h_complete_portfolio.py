...
# === Risk-Free Allocation Logic ===
def get_max_rf_ratio(score, A):
    # Ranh giới khẩu vị rủi ro
    if 10 <= score <= 17:
        range_min, range_max = 0.20, 0.40  # Rất bảo thủ
    elif 18 <= score <= 27:
        range_min, range_max = 0.10, 0.20  # Trung bình
    elif 28 <= score <= 40:
        range_min, range_max = 0.00, 0.10  # Ưa rủi ro
    else:
        raise ValueError("Risk score must be in [10, 40]")

    # Nội suy mềm dựa trên A
    A_clipped = np.clip(A, 2, 25)
    scale = (25 - A_clipped) / (25 - 2)
    return round(range_min + scale * (range_max - range_min), 4)
...
def run(hrp_result_dict, adj_returns_combinations, cov_matrix_dict,
    ...
    if mu_p <= 0 or sigma_p <= 0:
        raise ValueError("Selected portfolio has non-positive return or risk.")

    # 3. Tính max tỷ trọng tài sản phi rủi ro
