...
# --- BLOCK A: Data Collection and Preparation ---
...
def run(tickers, benchmark_symbol, start_date, end_date):
    ...
    def get_stock_data(ticker, start=start_date, end=end_date):
        try:
            stock = Vnstock().stock(symbol=ticker, source='VCI')
            df = stock.quote.history(start=start.strftime('%Y-%m-%d'), end=end.strftime('%Y-%m-%d'))
            if df.empty or 'close' not in df.columns or 'volume' not in df.columns:
                warnings.warn(f"{ticker} missing 'close' or 'volume' column.")
                return pd.DataFrame()
    ...
    def load_all_monthly_data(tickers):
        stock_data = []
    ...
    if data_stocks.empty or data_benchmark.empty:
        raise ValueError("Stock or benchmark data could not be loaded.")
