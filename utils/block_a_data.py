...
def get_stock_data(ticker, start, end):
    try:
        ...
        st.warning(f"Lỗi khi lấy dữ liệu cho {ticker}: {e}")
        return pd.DataFrame()
...
def run(tickers, benchmark_symbol, start_date, end_date):
    ...
    df_stocks = load_all_monthly_data(tickers, start_date, end_date)
    df_benchmark = load_all_monthly_data([benchmark_symbol], start_date, end_date)

    if df_stocks.empty:
        raise ValueError("Không có mã cổ phiếu nào có dữ liệu hợp lệ.")
    if df_benchmark.empty:
        raise ValueError(f"Không có dữ liệu cho benchmark: {benchmark_symbol}")

    returns_stocks = compute_monthly_return(df_stocks)
    ...
    # Merge benchmark vào stock returns
    ...
    return df_stocks, df_benchmark, returns_pivot_stocks, returns_benchmark, portfolio_combinations
