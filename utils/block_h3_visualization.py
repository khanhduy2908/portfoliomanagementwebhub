...
def run(best_portfolio, mu_p, sigma_p, rf, sigma_c, expected_rc, y_capped, y_opt, tickers, weights, cov):
    ...
    # === Step 1: Simulate portfolios from client-selected stocks ===
    n_simulations = 4000
    np.random.seed(42)
    weights_sim = np.random.dirichlet(np.ones(len(tickers)), size=n_simulations)
    
    mu_vec = np.array([mu_p] * len(tickers))
    ...
    # === Step 2: Plot Efficient Frontier ===
    fig, ax = plt.subplots(figsize=(10, 7), facecolor="#121212")

    sc = ax.scatter(sigma_sim * 100, mu_sim * 100, c=sharpe_sim,
    ...
    # === Step 4: Capital Allocation Line (CAL) ===
    slope = (mu_p - rf) / sigma_p
    x_cal = np.linspace(0, max(sigma_sim.max(), sigma_p) * 1.2, 100)
