...
def run(best_portfolio, mu_p, sigma_p, rf, sigma_c, expected_rc, y_capped, y_opt, tickers, weights, cov, simulate_for_visual=True):
    ...
    if simulate_for_visual:
        ...
        # === Estimate realistic expected returns for simulation ===
        if isinstance(best_portfolio, dict) and 'mu' in best_portfolio:
            mu_realistic = np.array(best_portfolio['mu'])
        else:
            st.error("⚠️ best_portfolio must be a dictionary with 'mu' key.")
            return
