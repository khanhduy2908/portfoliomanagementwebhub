...
def engineer_features(df, ticker_id):
    ...
    df['Volatility_Close'] = df['Close'].rolling(3).std()
        ...
        for i in range(lookback, len(df_ticker)):
            ...
            for lag in reversed(range(lookback)):
                for col in feature_cols:
                    row.append(df_ticker[col].iloc[i - lag])
            X.append(row)
...
def run(data_stocks, selected_tickers, selected_combinations):
    ...
    if not features_all:
        raise ValueError("❌ No valid stock for forecasting.")
    features_df = pd.concat(features_all, ignore_index=True)

    os.makedirs("saved_models", exist_ok=True)
    forecast_valid_combos = []

    # Step 2: Train per combination
    for combo in selected_combinations:
        subset = tuple(combo)
        if not all(t in valid_tickers for t in subset):
            continue

        combo_str = '-'.join(sorted(subset))
        hash_key = hashlib.md5(combo_str.encode()).hexdigest()
        ...
        if len(X_raw) < min_samples:
            warnings.warn(f"{subset}: too few samples.")
            continue
    ...
    if not adj_returns_combinations:
        raise ValueError("❌ No forecastable portfolio combinations found.")
