...
def run(data_stocks, benchmark_data, benchmark_symbol, portfolio_weights, tickers,
        start_date, end_date, rf=0.0, allocations_time=None):
    ...
    try:
        ...
        # Lọc dữ liệu theo khoảng thời gian người dùng nhập
        ...
        # Pivot dữ liệu giá theo ngày và ticker
        ...
        # Đồng bộ ngày giữa stocks và benchmark
        ...
        # Đồng bộ ticker giữa returns và weights
        common_tickers = list(set(returns_stocks.columns).intersection(tickers))
        if not common_tickers:
            st.warning("No common tickers between price data and portfolio weights.")
            return

        returns_stocks = returns_stocks[common_tickers]
        weights = np.array([portfolio_weights[tickers.index(t)] for t in common_tickers])

        # Tính lợi nhuận danh mục
        port_returns = returns_stocks.dot(weights)

        # Rolling Beta (30 ngày window)
        window = 30
        ...
        # Rolling Volatility và Rolling Sharpe (annualized)
        ...
        # Xử lý Asset Allocation Over Time nếu có dữ liệu truyền vào
        alloc_pivot = None
        if allocations_time is not None and not allocations_time.empty:
            required_cols = {'time', 'Ticker', 'Allocation'}
            if not required_cols.issubset(set(allocations_time.columns)):
                st.warning(f"allocations_time missing required columns: {required_cols}")
            else:
                allocations_time['time'] = pd.to_datetime(allocations_time['time'])
                allocations_time = allocations_time.loc[
                    (allocations_time['time'] >= pd.to_datetime(start_date)) &
                    (allocations_time['time'] <= pd.to_datetime(end_date))
                ]
        ...
        fig.add_trace(go.Scatter(x=rolling_sharpe.index, y=rolling_sharpe, mode='lines', name='Rolling Sharpe',
                                 line=dict(color='lime')), row=2, col=2)
        ...
        if alloc_pivot is not None:
            for ticker in alloc_pivot.columns:
                fig.add_trace(go.Bar(x=alloc_pivot.index, y=alloc_pivot[ticker], name=ticker), row=3, col=2)
            fig.update_layout(barmode='stack')
        else:
            fig.add_trace(go.Scatter(x=[None], y=[None], mode='markers',
                                     name="No allocation data", showlegend=False), row=3, col=2)
        ...
        # Đổi màu trục x,y thành trắng cho tất cả subplot
        ...
        st.error(f"Error in Portfolio Dynamics & Risk Analysis: {e}")
