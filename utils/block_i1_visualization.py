...
def run(returns_df, tickers, rf_monthly, start_date, end_date):
    ...
    with row1_col2:
        ...
        fig_dd = px.area(
            drawdown_reset,
            x='Date', y='Drawdown', color='Ticker',
            title='Drawdown (%)',
    ...
    with row2_col1:
        ...
        rolling_sharpe_reset = rolling_sharpe.reset_index().melt(id_vars='Date', var_name='Ticker', value_name='Rolling Sharpe Ratio')
        fig_rs = px.line(
            rolling_sharpe_reset,
            x='Date', y='Rolling Sharpe Ratio', color='Ticker',
    ...
    with row2_col2:
        ...
        rolling_vol_reset = rolling_volatility.reset_index().melt(id_vars='Date', var_name='Ticker', value_name='Rolling Volatility')
        fig_rv = px.line(
            rolling_vol_reset,
            x='Date', y='Rolling Volatility', color='Ticker',
